...
def Neural_Network(input_size, output_size):
    ...
    network = fully_connected(network, 5000, activation='relu')
    network = dropout(network, keep_prob)
    ...
    model = tflearn.DNN(network, tensorboard_verbose=3, tensorboard_dir='/tmp/tflearn_logs/')  # Creates the neural network
...
def simulation(env, model, force_vector, score_limit, N_runs, N_steps_per_run, save_data=False,
    ...
    if save_plots:
        ...
        hist_text_1 = '\n'.join((' Score limit: ' + str(score_limit),
                                 ...
                                 "Median: " + str(round(median(scores), 1)), "Mean time length per run: "
                                 + str(round(mean(runs_length), 1))))
...
if __name__ == "__main__":
    ...
    file_name = "CartPole v12_4_BigNN"  # For saving!
    open_file = "CartPole v12_2"
    Info_Text = ""
    Create_new_model = False
    create_new_model_on_old_data = True
    ...
    N_runs_1 = 1000000  # Number of runs per
