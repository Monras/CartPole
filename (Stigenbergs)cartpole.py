...
class DQNAgent:
    ...
    def __init__(self, state_size, action_size):
        ...
        self.target_update_frequency = 7
    ...
    def print_config(self):
        ...
        learning_rate: {}
        memory_size: {}
        target_update_freq: {}""".format(self.discount_factor, self.learning_rate, self.memory_size, self.target_update_frequency))
    ...
    def get_action(self, state):
        ...
        # Generate random action with probability epsilon
        if random.random() < self.epsilon:
            return random.randrange(self.action_size)

        # Else, return what is believed to be the best action
        Q = self.model.predict(state).reshape(-1)
        return np.argmax(Q)
    ...
    def train_model(self):
        ...
        for i in range(self.batch_size): #For every batch
            next_Q = 0 if done[i] else np.max(target_val[i,:])
            target[i][action[i]] = reward[i] + self.discount_factor * next_Q
...
if __name__ == "__main__":
    ...
    print(agent.build_model().to_yaml())
        ...
        while not done:
            ...
            if done:
                ...
                if agent.check_solve:
                    avg = np.mean(scores[-min(100, len(scores)):])
                    print(avg)
                    if avg >= 195:
                        ...
                        solved = True
                        break
                        # agent.plot_data(episodes,moving_average(scores,n=10),max_q_mean[:e+1])
                        # sys.exit()
    
    agent.plot_data(episodes,moving_average(scores,n=10),max_q_mean)
    agent.print_config()
    print(agent.build_model().to_yaml())
