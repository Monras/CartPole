...
# imports packages
from gym_kod_v11 import CartPoleEnv as gym
...
def simulation(env, model, theta_lim, force_vector, score_limit, N_runs, N_steps_per_run, save_data=False,
...
def see_training(env, training_data, theta_lim, N_runs, N_steps_per_run):
    div = N_runs / 10
    j = 0
    for run in range(N_runs):
        env.reset(theta_lim)
...
if __name__ == "__main__":
    ...
    file_name = "CartPole v10_4_1"  # For saving!
    ...
    if Create_new_model:
        ...
        # see_training(env, training_data, theta_limit_1, 2, N_steps_per_run_1)
    ...
    if load_model_and_run:
        ...
        training_data = simulation(env, model, theta_limit_sim, force_vector, score_limit_sim, N_runs_sim,
                                   N_steps_per_run_sim,
                                   save_data=False, save_plots=True, animation=False,
                                   create_training_data=False, run_simulation=True,
    ...
    if create_new_model_on_old_data:
        ...
        training_data_trained = simulation(env, model, theta_limit_sim, force_vector, score_limit_sim, N_runs_sim,
                                   N_steps_per_run_sim,
